...
def _evaluate(
    ...
    for i, row in enumerate(dataset, start=1):
        ...
        disease_hint = (
            str(row.get("disease_hint", ""))
            if row.get("disease_hint") is not None
            else ""
        )
        ...
        if use_filter:
            retrieved = search(
                query_vec, top_k=top_k, crop=crop or None, disease_hint=disease_hint or None
            )
        else:
...
def main() -> None:
    ...
    if args.mode in ("filtered", "both"):
        report["filtered"] = _evaluate(dataset, top_k=args.top_k, use_filter=True)
    if args.mode in ("unfiltered", "both"):
        report["unfiltered"] = _evaluate(dataset, top_k=args.top_k, use_filter=False)
